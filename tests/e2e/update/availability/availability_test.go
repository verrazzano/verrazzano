...
var _ = t.Describe("Status", func() {
	...
	t.It("dynamically scales availability", func() {
		...
		t.Logs.Infof("Scale up the target deployment %s/%s", deployNamespace, deployName)
		scaleDeployment(deployName, deployNamespace, 3)
		t.Logs.Info("After the health check updates the status, all components should be available")
		hasUnavailableComponents(0)
	})
})
