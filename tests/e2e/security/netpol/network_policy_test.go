		...
		pkg.Concurrently(
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test cert-manager ingress failed: reason = %s", err))
				}, longWaitTimeout, pollingInterval).Should(Succeed())
			},
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test ingress-nginx-controller ingress failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test ingress-nginx-default-backend ingress failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test istiod-verrazzano-system ingress failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test keycloak ingress rules failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test mysql ingress rules failed: reason = %s", err))
				}, longWaitTimeout, pollingInterval).Should(Succeed())
			},
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test verrazzano-platform-operator ingress rules failed: reason = %s", err))
				}, longWaitTimeout, pollingInterval).Should(Succeed())
			},
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test coherence-operator ingress rules failed: reason = %s", err))
					// Allowing pods to be optional because some contexts in which this test is run disables the Coherence operator.
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test verrazzano-application-operator ingress rules failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test verrazzano-authproxy ingress rules failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test verrazzano-console ingress rules failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test node-exporter ingress rules failed: reason = %s", err))
				}, longWaitTimeout, pollingInterval).Should(Succeed())
			},
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test istio-ingressgateway ingress rules failed: reason = %s", err))
				}, longWaitTimeout, pollingInterval).Should(Succeed())
			},
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test istio-egressgateway ingress rules failed: reason = %s", err))
				}, longWaitTimeout, pollingInterval).Should(Succeed())
			},
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test vmi-system-es-master ingress rules failed: reason = %s", err))
				}, longWaitTimeout, pollingInterval).Should(Succeed())
				/* TODO:
			...
			*/
			...
			// 	g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test vmi-system-es-data ingress rules failed: reason = %s", err))
			...
			// 	g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test vmi-system-es-ingest ingress rules failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test vmi-system-grafana ingress rules failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test vmi-system-kibana ingress rules failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test vmi-system-prometheus ingress rules failed: reason = %s", err))
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test weblogic-operator ingress rules failed: reason = %s", err))
					// Allowing pods to be optional because some contexts in which this test is run disables the WebLogic operator.
			...
			func() {
				Eventually(func(g Gomega) {
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test Kiali network ingress from verrazzano-authproxy failed: reason = %s", err))
					...
					g.Expect(err).To(BeNil(), fmt.Sprintf("FAIL: Test Kiali network ingress from prometheus failed: reason = %s", err))
				}, longWaitTimeout, pollingInterval).Should(Succeed())
			},
		)
	})
