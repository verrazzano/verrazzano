...
const (
	...
	pgClientSecret   = "verrazzano-pg"   //nolint:gosec //#gosec G101
)
...
var _ = t.Describe("Dex", Label("f:platform-lcm.install"), func() {
	t.Context("after successful installation", func() {
		...
		// GIVEN the Dex is installed
		// WHEN we check to make sure the PG secret is created
		// THEN we successfully find the secret in the cluster
		WhenDexInstalledIt("PG secret exists", func() {
			Eventually(func() bool {
				result, err := pkg.DoesSecretExist(constants.DexIngress, pgClientSecret)
				if err != nil {
					AbortSuite(fmt.Sprintf("Secret %s does not exist in the namespace: %v, error: %v", pkceClientSecret, constants.DexIngress, err))
				}
				return result
			}, waitTimeout, pollingInterval).Should(BeTrue(), "Failed to find PG secret")
		})
	})
})
