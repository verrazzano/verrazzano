...
var _ = t.Describe("Post upgrade OpenSearch", Label("f:observability.logging.es"), func() {
	...
	MinimumVerrazzanoIt("If ISM already exists, then also default policy created.", func() {
		Eventually(func() bool {
			...
			return true
		}).WithPolling(pollingInterval).WithTimeout(waitTimeout).Should(BeTrue(), "Expected not to find any old policies")
	})
	...
	t.ItMinimumVersion("Verify OpenSearch plugins have been installed", "1.5.3", kubeConfigPath, func() {
