...
func main() {
	...
	// Convert the MetricsTrait to a ServiceMonitor
	err = yaml.Unmarshal(appData, metricsTrait)
	if err == nil {
		serviceMonitor := handleMetricsTrait(metricsTrait)

		// Marshal and write the ServiceMonitor to a file
		serviceMonitorYAML, err := yaml.Marshal(serviceMonitor)
		if err != nil {
			fmt.Printf("Error while Marshaling. %v", err)
		}

		fileName := "/path/to/output/directory/" + serviceMonitor.Name + ".yaml"
		err = ioutil.WriteFile(fileName, serviceMonitorYAML, 0644)
		if err != nil {
