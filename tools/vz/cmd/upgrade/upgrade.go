...
func runCmdUpgrade(cmd *cobra.Command, vzHelper helpers.VZHelper) error {
	...
	if vz.Spec.Version == "" || !upgradeVersion.IsEqualTo(vzSpecVersion) {
		...
		return waitForUpgradeToComplete(client, kubeClient, vzHelper, types.NamespacedName{Namespace: vz.Namespace, Name: vz.Name}, timeout, logFormat)
	}
...
func waitForUpgradeToComplete(client clipkg.Client, kubeClient kubernetes.Interface, vzHelper helpers.VZHelper, namespacedName types.NamespacedName, timeout time.Duration, logFormat cmdhelpers.LogFormat) error {
	return cmdhelpers.WaitForOperationToComplete(client, kubeClient, vzHelper, namespacedName, timeout, logFormat, v1beta1.CondUpgradeComplete)
}
