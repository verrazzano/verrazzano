...
2. Modify the [worker Chart.yaml](./manifests/charts/worker/Chart.yaml) file and add a dependency for the mysql chart
```
dependencies:
  - name: mysql
...
Worker metrics are very important and how we track the progress of a worker.  Before implementing the `DoWork` and `PreconditionsMet` functions,
