...
func TestPrerequisiteValidationWarningForV1beta1(t *testing.T) {
	...
	asrt.Contains(res.Warnings[0], "minimum required number of worker nodes for prod profile is 3 but the available number of worker nodes is 1")
	asrt.Contains(res.Warnings[1], "minimum required CPUs for prod profile is 4 but the CPUs on node node1 is 3")
	asrt.Contains(res.Warnings[2], "minimum required memory for prod profile is 32G but the memory on node node1 is 16G")
}
