...
type ElasticsearchComponent struct {
	...
	// A list of values to use during OpenSearch installation. Each argument is specified as either a `name/value` or
...
type AuthProxyComponent struct {
	...
	// +optional
	InstallOverrides `json:",inline"`
	// The Kubernetes resources than can be configured for AuthProxy.
	// +optional
...
type IstioIngressSection struct {
	...
	// Istio ingress type is NodePort, a valid and accessible IP address must be specified using the
	...
	// +optional
	Ports []corev1.ServicePort `json:"ports,omitempty"`
	// The Kubernetes resources than can be configured for an Istio ingress gateway.
	// +optional
...
type IstioEgressSection struct {
	// The Kubernetes resources than can be configured for an Istio egress gateway.
	// +optional
	Kubernetes *IstioKubernetesSection `json:"kubernetes,omitempty"`
}
