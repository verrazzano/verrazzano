...
## Building the Verrazzano platform operator
...
- To change the `deploy/operator.yaml` file, do not edit the file directly because it is generated by concatenating the following files:
  ...
  - `config/crd/bases/install.verrazzano.io_verrazzanos.yaml` - this file contains the YAML file to define the custom resource named `verrazzano`.
...
## Testing the Verrazzano platform operator
...
also that `$HOME/.kube/config` is empty. It will ensure that the tests
