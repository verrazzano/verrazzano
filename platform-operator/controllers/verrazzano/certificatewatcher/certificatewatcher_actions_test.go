...
func TestExpiredValidateCertificate(t *testing.T) {
	...
	certificateCheck, err := NewCertificateWatcher(fakeCtx.Client(), checkPeriodDuration, VZNamespace, VZNamespace, VZWebhookDeployment)
	assert.NoError(t, err)
	err = certificateCheck.WatchCertificates([]string{VZCertificate})
	assert.NoError(t, err)
}
...
func TestCertificate1(t *testing.T) {
	...
	cli := fake.NewClientBuilder().WithScheme(testScheme).WithObjects(&deployment, &secret).Build()
	fakeCtx := spi.NewFakeContext(cli, nil, nil, false)
	certificateCheck, err := NewCertificateWatcher(fakeCtx.Client(), checkPeriodDuration, VZNamespace, VZNamespace, "operator")
	assert.NoError(t, err)
	err = certificateCheck.WatchCertificates([]string{VZCertificate})
	assert.Equal(t, "either deployment not found or an error while restarting the deployment", err.Error())
}
...
func TestCertificate2(t *testing.T) {
	...
	cli := fake.NewClientBuilder().WithScheme(testScheme).WithObjects(&deployment, &secret).Build()
	fakeCtx := spi.NewFakeContext(cli, nil, nil, false)
	certificateCheck, err := NewCertificateWatcher(fakeCtx.Client(), checkPeriodDuration, VZNamespace, "verrazzano", "verrazzano-operator")
	assert.NoError(t, err)
	err = certificateCheck.WatchCertificates([]string{VZCertificate})
	assert.Equal(t, "either deployment not found or an error while restarting the deployment", err.Error())
}
...
// if secret to watch doesn't exist
// THEN error should be thrown
func TestCertificate3(t *testing.T) {
	...
	certificateCheck, err := NewCertificateWatcher(fakeCtx.Client(), checkPeriodDuration, VZNamespace, VZNamespace, VZWebhookDeployment)
	assert.NoError(t, err)
	err = certificateCheck.WatchCertificates([]string{"test-certificate"})
	assert.Equal(t, "an error while listing the certificate secret data", err.Error())
}
...
func TestCertificate4(t *testing.T) {
	...
	cli := fake.NewClientBuilder().WithScheme(testScheme).WithObjects(&deployment, &secret).Build()
	fakeCtx := spi.NewFakeContext(cli, nil, nil, false)
	certificateCheck, err := NewCertificateWatcher(fakeCtx.Client(), checkPeriodDuration, "verrazzano", VZNamespace, VZWebhookDeployment)
	assert.NoError(t, err)
	err = certificateCheck.WatchCertificates([]string{VZCertificate})
	assert.Equal(t, "an error while listing the certificate secret data", err.Error())
}
...
func newTestCertificateCheck() *CertificateWatcher {
	c := fake.NewClientBuilder().WithScheme(testScheme).Build()
	certificateWatcher, _ := NewCertificateWatcher(c, 2*time.Second, VZNamespace, VZNamespace, VZWebhookDeployment)
