		...
		for _, module := range localCatalog.Modules {
			...
			if checkIfModuleUpdated(t, module, localBOM, remoteBOM) {
				...
				assert.Truef(t, localVersion.IsGreatherThan(remoteVersion),
					"BOM entry for module %s has been updated so local catalog version %s should be greater than remote catalog version %s on target branch %s."+
					...
					module.Name, localVersion.ToString(), remoteVersion.ToString(), targetBranch, module.Name)
			}
		}
	}
}
