...
CREATE TABLE IF NOT EXISTS DATABASECHANGELOG (
  ...
  PRIMARY KEY (ID,AUTHOR,FILENAME)
	...
	mySQLRootCommand = `bash -c /usr/bin/mysql -uroot -p%s -e "GRANT ALL PRIVILEGES ON *.* TO 'root'@'localhost';"`
	mySQLDbCommands  = `bash -c /usr/bin/mysqlsh -uroot -p%s --py --execute "
...
`
	...
	mySQLShCommands = `bash -c /usr/bin/mysqlsh -uroot -p%s --js --execute 'util.dumpInstance("/var/lib/mysql/dump", {ocimds: true, compatibility: ["strip_definers", "strip_restricted_grants"]})'
`
