...
CREATE TABLE IF NOT EXISTS DATABASECHANGELOG (
  ...
  PRIMARY KEY (ID,AUTHOR,FILENAME)
	...
	mySQLDbCommands  = `LC_ALL=C.utf8 /usr/bin/mysqlsh -uroot -p%s --py --execute "
...
`
	...
	mySQLShCommands = `LC_ALL=C.utf8 /usr/bin/mysqlsh -uroot -p%s --js --execute 'util.dumpInstance("/var/lib/mysql/dump", {ocimds: true, compatibility: ["strip_definers", "strip_restricted_grants"]})'
`
