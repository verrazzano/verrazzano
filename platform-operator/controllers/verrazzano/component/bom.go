...
type Bom struct {
	// Registry it the top level registry URI which contains the image repositories.
	// An example is ghcr.io
	Registry string  `json:"registry"`

	// Components is the array of component boms in the bom
	Components []ComponentBom `json:"components"`
}

// ComponentBom represents a high level component, such as Istio.
...
type ComponentBom struct {
	...
	SubComponents []SubComponentBom   `json:"subcomponents"`
}

// SubComponentBom contains the bom information for a single helm chart.
// Istio is an example of a component with several sub-component.
type SubComponentBom struct {
	Name string  `json:"name"`
	...
	Images []ImageBom `json:"name"`
}

// ImageBom describes a single image used by one of the helm charts.  This structure
