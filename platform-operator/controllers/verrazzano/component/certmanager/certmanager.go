...
func (c certManagerComponent) PreInstall(compContext spi.ComponentContext) error {
	...
	if _, err := controllerutil.CreateOrUpdate(context.TODO(), compContext.Client(), &ns, func() error {
		...
		return compContext.Log().LogAndRetErrorf("Failed to create or update the cert-manager namespace: %s", err)
	}
	...
	if err != nil {
		return compContext.Log().LogAndRetErrorf("Failed to apply the cert-manager manifest: %v", err)
	}
	return nil
}
...
func (c certManagerComponent) PostInstall(compContext spi.ComponentContext) error {
	...
	if err != nil {
		return compContext.Log().LogAndRetErrorf("Failed to verify the config type: %s", err)
	}
	if !isCAValue {
		// Create resources needed for Acme certificates
		err := createAcmeResources(compContext)
		if err != nil {
			return compContext.Log().LogAndRetErrorf("Failed creating Acme resources: %v", err)
		}
	} else {
		// Create resources needed for CA certificates
		err := createCAResources(compContext)
		if err != nil {
			return compContext.Log().LogAndRetErrorf("Failed creating CA resources: %v", err)
		}
	}
	return nil
}
...
func (c certManagerComponent) applyManifest(compContext spi.ComponentContext) error {
	...
	if err != nil {
		return compContext.Log().LogAndRetErrorf("Failed writing CRD Manifests for CertManager: %v", err)
	}

	// Apply the CRD Manifest for CertManager
	if err = k8sutil.NewYAMLApplier(compContext.Client()).ApplyF(outputFile); err != nil {
		return compContext.Log().LogAndRetErrorf("Failed applying CRD Manifests for CertManager: %v", err)
	}
...
func AppendOverrides(compContext spi.ComponentContext, _ string, _ string, _ string, kvs []bom.KeyValue) ([]bom.KeyValue, error) {
	...
	isCAValue, err := isCA(compContext)
	if err != nil {
		err = compContext.Log().LogAndRetErrorf("Failed to verify the config type: %s", err)
...
func createAcmeResources(compContext spi.ComponentContext) error {
	...
	if err := compContext.Client().Get(context.TODO(), client.ObjectKey{Name: ociDNSConfigSecret, Namespace: namespace}, &secret); err != nil {
		return compContext.Log().LogAndRetErrorf("Failed to retireve the OCI DNS config secret: %s", err)
	}
	...
	if err != nil {
		return compContext.Log().LogAndRetErrorf("Failed to parse the ClusterIssuer yaml template: %s", err)
	}

	// Execute the template object with the given data
	err = template.Execute(&buff, &clusterIssuerData)
	if err != nil {
		return compContext.Log().LogAndRetErrorf("Failed to execute the ClusterIssuer template: %s", err)
	}
	...
	if err := yaml.Unmarshal(buff.Bytes(), ciObject); err != nil {
		return compContext.Log().LogAndRetErrorf("Failed to unmarshal yaml: %s", err)
	}
	...
	if _, err := controllerutil.CreateOrUpdate(context.TODO(), compContext.Client(), ciObject, func() error {
		...
		return compContext.Log().LogAndRetErrorf("Failed to create or update the ClusterIssuer: %s", err)
	}
	return nil
}
...
func createCAResources(compContext spi.ComponentContext) error {
	...
	if _, err := controllerutil.CreateOrUpdate(context.TODO(), compContext.Client(), &issuer, func() error {
		...
		return compContext.Log().LogAndRetErrorf("Failed to create or update the Issuer: %s", err)
	}
	...
	if _, err := controllerutil.CreateOrUpdate(context.TODO(), compContext.Client(), &certObject, func() error {
		...
		return compContext.Log().LogAndRetErrorf("Failed to create or update the Certificate: %s", err)
	}
