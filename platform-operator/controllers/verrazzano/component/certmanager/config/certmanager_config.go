...
func checkRenewAllCertificates(log vzlog.VerrazzanoLogger, cli crtclient.Client, config cmcommon.CertManagerConfiguration, isCAConfig bool) error {
	...
	issuerCNs, err := findIssuerCommonName(config.Certificate, isCAConfig)
	if err != nil {
		return err
	}
...
func (c certManagerConfigComponent) cmCRDsExist(log vzlog.VerrazzanoLogger) bool {
	crdsExist, err := cmcommon.CertManagerCrdsExist()
	if err != nil {
...
func createOrUpdateAcmeResources(log vzlog.VerrazzanoLogger, client crtclient.Client, vz *vzapi.Verrazzano, config cmcommon.CertManagerConfiguration) (opResult controllerutil.OperationResult, err error) {
...
func createACMEIssuerObject(log vzlog.VerrazzanoLogger, client crtclient.Client, vz *vzapi.Verrazzano, config cmcommon.CertManagerConfiguration) (*unstructured.Unstructured, error) {
	...
	vzCertAcme := config.Certificate.Acme
	...
	if cmcommon.IsLetsEncryptStaging(vzCertAcme) {
...
// - returns OperationResultCreated/nil if the CI is created (initial install)
// - returns OperationResultUpdated/nil if the CI is updated
func createOrUpdateCAResources(log vzlog.VerrazzanoLogger, crtClient crtclient.Client, config cmcommon.CertManagerConfiguration) (controllerutil.OperationResult, error) {
	vzCertCA := config.Certificate.CA
...
func cleanupUnusedResources(log vzlog.VerrazzanoLogger, client crtclient.Client, config cmcommon.CertManagerConfiguration, isCAValue bool) error {
	defaultCANotUsed := func() bool {
		...
		return !isCAValue || config.Certificate.CA.SecretName != cmcommon.DefaultCACertificateSecretName
	}
	if isCAValue {
		log.Oncef("Clean up ACME issuer secret")
		// clean up ACME secret if present
		if err := deleteObject(client, caAcmeSecretName, config.ClusterResourceNamespace, &v1.Secret{}); err != nil {
			return err
		}
	}
	if defaultCANotUsed() {
		...
		if err := deleteObject(client, caSelfSignedIssuerName, config.ClusterResourceNamespace, &certv1.Issuer{}); err != nil {
			return err
		}
		if err := deleteObject(client, caCertificateName, config.ClusterResourceNamespace, &certv1.Certificate{}); err != nil {
			return err
		}
		if err := deleteObject(client, cmcommon.DefaultCACertificateSecretName, config.ClusterResourceNamespace, &v1.Secret{}); err != nil {
			return err
		}
	}
	return nil
...
func (c certManagerConfigComponent) createOrUpdateClusterIssuer(compContext spi.ComponentContext) error {
	cmConfig, err := cmcommon.GetCertManagerConfiguration(compContext.EffectiveCR())
	if err != nil {
		return err
	}
	isCAValue, err := cmcommon.IsCAConfig(cmConfig.Certificate)
	if err != nil {
	...
	if !isCAValue {
		...
		if opResult, err = createOrUpdateAcmeResources(compContext.Log(), compContext.Client(), compContext.EffectiveCR(), cmConfig); err != nil {
			return compContext.Log().ErrorfNewErr("Failed creating Acme resources: %v", err)
		}
	} else {
		// Create resources needed for CA certificates
		if opResult, err = createOrUpdateCAResources(compContext.Log(), compContext.Client(), cmConfig); err != nil {
	...
	if err := cleanupUnusedResources(compContext.Log(), compContext.Client(), cmConfig, isCAValue); err != nil {
		return err
	}
	if err := checkRenewAllCertificates(compContext.Log(), compContext.Client(), cmConfig, isCAValue); err != nil {
...
func (c certManagerConfigComponent) uninstallVerrazzanoCertManagerResources(compContext spi.ComponentContext) error {
	...
	if err := c.deleteCertManagerIssuerResources(compContext.Log(), compContext.Client(), cmConfig); err != nil {
		return err
	}
...
func (c certManagerConfigComponent) deleteCertManagerIssuerResources(log vzlog.VerrazzanoLogger, client crtclient.Client, config cmcommon.CertManagerConfiguration) error {

	// Delete the ClusterIssuer created by Verrazzano
	if !c.cmCRDsExist(log) {
		log.Progressf("CertManager CRDs do not exist, skipping ClusterIssuer cleanup")
		return nil
	}
