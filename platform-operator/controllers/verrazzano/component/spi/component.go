...
type ComponentContext interface {
	...
	// IsDryRun indicates the componentName context is in DryRun mode
	IsDryRun() bool
	// Copy returns a copy of the current context
	Copy() ComponentContext
	// GetOperation returns the operation object in the context
	GetOperation() string
	// GetComponent returns the componentName object in the context
	GetComponent() string
}

// ComponentInfo interface defines common information and metadata about components
type ComponentInfo interface {
	// Name returns the name of the Verrazzano componentName
	Name() string
	// GetDependencies returns the dependencies of this componentName
	GetDependencies() []string
	// IsReady Indicates whether or not a componentName is available and ready
	IsReady(context ComponentContext) bool
	// IsEnabled Indicates whether or a componentName is enabled for installation
	IsEnabled(context ComponentContext) bool
	// GetMinVerrazzanoVersion returns the minimum Verrazzano version required by the componentName
	GetMinVerrazzanoVersion() string
	// GetIngressNames returns a list of names of the ingresses associated with the componentName
...
type ComponentInstaller interface {
	// IsOperatorInstallSupported Returns true if the componentName supports install directly via the platform operator
	// - scaffolding while we move components from the scripts to the operator
	IsOperatorInstallSupported() bool
	// IsInstalled Indicates whether or not the componentName is installed
	...
	// Install performs the initial install of a componentName
...
type ComponentUpgrader interface {
	...
	// Upgrade will upgrade the Verrazzano componentName specified in the CR.Version field
