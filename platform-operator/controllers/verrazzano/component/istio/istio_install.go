...
func (i istioComponent) PostInstall(compContext spi.ComponentContext) error {
	...
	if err := restart.RestartComponents(compContext.Log(), config.GetInjectedSystemNamespaces(), compContext.ActualCR().Generation, restart.DoesPodContainNoIstioSidecar); err != nil {
		return err
	}
