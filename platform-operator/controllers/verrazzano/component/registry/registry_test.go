...
func TestGetComponents(t *testing.T) {
	...
	i++
	a.Equal(comps[i].Name(), common.IstioComponentName)
	i++
...
func TestFindComponent(t *testing.T) {
	found, comp := FindComponent(common.IstioComponentName)
	assert.True(t, found)
	assert.NotNil(t, comp)
	assert.Equal(t, common.IstioComponentName, comp.Name())
}
...
func TestComponentDependenciesNotMet(t *testing.T) {
	comp := helm2.HelmComponent{
		...
		ChartDir:       "chartDir",
		ChartNamespace: "bar",
		Dependencies:   []string{common.IstioComponentName},
	}
...
func TestComponentMultipleDependenciesPartiallyMet(t *testing.T) {
	comp := helm2.HelmComponent{
		...
		ChartDir:       "chartDir",
		ChartNamespace: "bar",
		Dependencies:   []string{common.IstioComponentName, "cert-manager"},
	}
	client := fake.NewClientBuilder().WithScheme(k8scheme.Scheme).WithObjects(&appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Namespace: "istio-system",
...
func (f fakeComponent) GetModuleConfigAsHelmValues(effectiveCR *v1alpha1.Verrazzano) (*apiextensionsv1.JSON, error) {
	return nil, nil
}
