...
func AppendOverrides(ctx spi.ComponentContext, _ string, _ string, _ string, kvs []bom.KeyValue) ([]bom.KeyValue, error) {
	...
	kvs = append(kvs, bom.KeyValue{Key: fmt.Sprintf("commonLabels.%s", vzconst.VerrazzanoComponentLabelKey), Value: ComponentName})
...
func appendTLSSecretGeneratorImage(ctx spi.ComponentContext, kvs []bom.KeyValue) ([]bom.KeyValue, error) {
	...
	if len(images) > 0 {
		kvs = append(kvs, []bom.KeyValue{
			...
			{Key: "prometheus.prometheusSpec.containers[1].name", Value: constants.VZTLSSecGeneratorName},
