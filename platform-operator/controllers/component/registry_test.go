...
func TestGetComponents(t *testing.T) {
	...
	assert.Equal(comps[1].Name(), "ingress-controller")
	...
	assert.Equal(comps[7].Name(), "weblogic-operator")
	assert.Equal(comps[8].Name(), "oam-kubernetes-runtime")
	assert.Equal(comps[9].Name(), "verrazzano-application-operator")
	assert.Equal(comps[10].Name(), "mysql")
	assert.Equal(comps[11].Name(), "keycloak")
}
