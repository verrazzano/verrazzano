...
if [ -z "$OPERATOR_YAML" ] && [ "" = "${OPERATOR_YAML}" ]; then
  ...
  if [ "NONE" = "${VERRAZZANO_OPERATOR_IMAGE}" ]; then
      ...
      oci --region us-phoenix-1 os object get --namespace ${OCI_OS_NAMESPACE} -bn ${OCI_OS_BUCKET} --name master/ef94503f/operator.yaml --file ${WORKSPACE}/downloaded-operator.yaml
