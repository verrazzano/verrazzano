...
if [ -f "$WORKSPACE/vz" ]; then
  cd $WORKSPACE
  ./vz install --filename ${WORKSPACE}/acceptance-test-config.yaml --wls-file ${TARGET_OPERATOR_FILE} --timeout ${INSTALL_TIMEOUT_VALUE}
else
  cd ${GO_REPO_PATH}/verrazzano/tools/vz
  GO111MODULE=on GOPRIVATE=github.com/verrazzano go run main.go install --filename ${VZ_INSTALL_FILE} --wls-file ${TARGET_OPERATOR_FILE} --timeout ${INSTALL_TIMEOUT_VALUE}
fi
result=$?
if [[ $result -ne 0 ]]; then
  exit 1
