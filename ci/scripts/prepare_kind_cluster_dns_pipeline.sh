...
if [ -z "$JENKINS_URL" ] || [ -z "$GO_REPO_PATH" ] || [ -z "$TESTS_EXECUTED_FILE" ] || [ -z "$WORKSPACE" ] || [ -z "$VERRAZZANO_OPERATOR_IMAGE" ] \
   || [ -z "$INSTALL_CONFIG_FILE_KIND" ] || [ -z "$OCI_OS_LOCATION" ] || [ -z "$OCI_OS_COMMIT_BUCKET" ] \
   || [ -z "$INSTALL_CONFIG_FILE_OCIDNS" ] || [ -z "$DNS_SCOPE" ] || [ -z "$NGINX_LB_SCOPE" ] || [ -z "$ISTIO_LB_SCOPE" ] \
   || [ -z "$CERT_ISSUER" ] || [ -z "$ACME_ENVIRONMENT" ] ||  [ -z "$TEST_SCRIPTS_DIR" ]; then
...
cd ${TEST_SCRIPTS_DIR}
./process_oci_dns_install_yaml.sh $INSTALL_CONFIG_FILE_OCIDNS $CERT_ISSUER $ACME_ENVIRONMENT ${DNS_SCOPE} ${NGINX_LB_SCOPE} ${ISTIO_LB_SCOPE}
cd ${TEST_SCRIPTS_DIR}
./create-test-oci-config-secret.sh
