...
if [ -z "$JENKINS_URL" ] || [ -z "$BUILD_OS" ] || [ -z "$MODULE_REPO_ARCHIVE_DIR" ] ||
[ -z "$MODULE_STREAM_VERSION" ]; then
  echo "This script must only be called from Jenkins and requires a number of environment variables are set"
  exit 1
fi

cat <<EOF >"${WORKSPACE}/verrazzano-${BUILD_OS}.repo"
...
sudo mv "${WORKSPACE}/verrazzano-${BUILD_OS}.repo" "/etc/yum.repos.d/verrazzano-${BUILD_OS}.repo"
