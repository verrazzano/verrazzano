...
helm upgrade cert-manager -n cert-manager platform-operator/thirdparty/charts/cert-manager --set image.repository=ghcr.io/verrazzano/cert-manager-controller --set image.tag=${controllerTag}  --set cainjector.image.repository=ghcr.io/verrazzano/cert-manager-cainjector --set cainjector.image.tag=${cainjectorTag}  --set webhook.image.repository=ghcr.io/verrazzano/cert-manager-webhook --set webhook.image.tag=${webhookTag} --install
