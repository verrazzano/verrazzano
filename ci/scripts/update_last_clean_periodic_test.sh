...
oci --region ${OCI_OS_REGION} os object copy --namespace ${OCI_OS_NAMESPACE} -bn ${OCI_OS_COMMIT_BUCKET} --destination-bucket ${OCI_OS_BUCKET} --source-object-name ephemeral/${BRANCH_NAME}/${SHORT_COMMIT_HASH_ENV}/verrazzano_${DEVELOPMENT_VERSION}-images.txt --destination-object-name ${CLEAN_BRANCH_NAME}/verrazzano_${DEVELOPMENT_VERSION}-images.txt
