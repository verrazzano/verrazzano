...
if [ -z "$WORKSPACE" ] || [ -z "$OCI_OS_NAMESPACE" ] || [ -z "$OCI_OS_BUCKET" ] || [ -z "$OCIR_SCAN_REGISTRY" ] || [ -z "$OCIR_SCAN_REPOSITORY_PATH" ] || [ -z "$CLEAN_BRANCH_NAME" ]; then
...
# happening from the periodic tests. This allows folks to push images from their branches to OCIR.
echo "Pushing verrazzano-bom.json to object storage"
oci --region us-phoenix-1 os object put --force --namespace ${OCI_OS_NAMESPACE} -bn ${OCI_OS_BUCKET} --name ${CLEAN_BRANCH_NAME}/last-ocir-pushed-verrazzano-bom.json --file ${WORKSPACE}/verrazzano-bom.json
