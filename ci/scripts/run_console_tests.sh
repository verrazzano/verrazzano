...
if [[ -z $console_sha ]]; then
  ...
  console_tag=$(cat platform-operator/verrazzano-bom.json|jq -r '.components[] | select(.name == "verrazzano") | .subcomponents[] | select (.name == "verrazzano") | .images[] | select (.image == "console") | .tag')
