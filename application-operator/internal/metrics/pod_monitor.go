...
func PopulatePodMonitor(info ScrapeInfo, podMonitor *promoperapi.PodMonitor, workloadNamespace string, log vzlog.VerrazzanoLogger) error {
	...
	podMonitor.Spec.NamespaceSelector = promoperapi.NamespaceSelector{
		// The namespace of the workload is different from the namespace of the Pod Monitor
		MatchNames: []string{workloadNamespace},
	}
...
func createPodMonitorEndpoint(info ScrapeInfo, portIncrement int) (promoperapi.PodMetricsEndpoint, error) {
	...
	if info.IstioEnabled != nil && *info.IstioEnabled {
		...
		// These certs are generated by Istio and are created from the TLS secret generator
		// Pod Monitors reference a local secret and have to live in the verrazzano-monitoring namespace
