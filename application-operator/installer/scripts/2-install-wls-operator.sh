...
function install_wls_operator {
  ...
  git --git-dir=$TMP_DIR/weblogic-kubernetes-operator/.git --work-tree=$TMP_DIR/weblogic-kubernetes-operator checkout v3.2.3
  if [ $? -ne 0 ]; then
