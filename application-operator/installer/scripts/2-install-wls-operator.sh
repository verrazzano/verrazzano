...
function install_wls_operator {
  ...
  helm install weblogic-operator \
       $TMP_DIR/weblogic-kubernetes-operator/kubernetes/charts/weblogic-operator \
       --namespace verrazzano-system \
       --set image=ghcr.io/oracle/weblogic-kubernetes-operator:3.1.0 \
       --set serviceAccount=weblogic-operator-sa \
       --set domainNamespaceSelectionStrategy=LabelSelector \
       --set domainNamespaceLabelSelector=verrazzano-maanged \
