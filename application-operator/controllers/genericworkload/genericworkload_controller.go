...
const (
	labelKey       = "verrazzanogenericworkloads.oam.verrazzano.io"
	controllerName = "genericworkload"
)
...
// Reconciler reconciles a VerrazzanoGenericWorkload object
...
func (r *Reconciler) SetupWithManager(mgr ctrl.Manager) error {
	return ctrl.NewControllerManagedBy(mgr).
		For(&vzapi.VerrazzanoGenericWorkload{}).
		Owns(&appsv1.Deployment{}, builder.WithPredicates(predicate.GenerationChangedPredicate{})).
		Owns(&corev1.Service{}, builder.WithPredicates(predicate.GenerationChangedPredicate{})).
		Complete(r)
}

// Reconcile reconciles a VerrazzanoGenericWorkload resource. It fetches the embedded DeploymentSpec, mutates it to add
...
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	counterMetricObject, errorCounterMetricObject, reconcileDurationMetricObject, zapLogForMetrics, err := metricsexporter.ExposeControllerMetrics(controllerName, metricsexporter.GenericReconcileCounter, metricsexporter.GenericReconcileError, metricsexporter.GenericReconcileDuration)
	if err != nil {
	...
	if req.Namespace == vzconst.KubeSystem {
		...
		log.Infof("Generic workload resource %v should not be reconciled in kube-system namespace, ignoring", req.NamespacedName)
	...
	log, err := clusters.GetResourceLogger("verrazzanogenericworkload", req.NamespacedName, &workload)
	if err != nil {
		errorCounterMetricObject.Inc(zapLogForMetrics, err)
		zap.S().Errorf("Failed to create controller logger for Generic workload resource: %v", err)
		return clusters.NewRequeueWithDelay(), nil
	}
	log.Oncef("Reconciling Generic workload resource %v, generation %v", req.NamespacedName, workload.Generation)
	...
	log.Oncef("Finished reconciling Generic workload %v", req.NamespacedName)
	counterMetricObject.Inc(zapLogForMetrics, err)
	return ctrl.Result{}, nil
}

// doReconcile performs the reconciliation operations for the VerrazzanoGenericWorkload
func (r *Reconciler) doReconcile(ctx context.Context, workload vzapi.VerrazzanoGenericWorkload, log vzlog.VerrazzanoLogger) (ctrl.Result, error) {
	...
	if len(workload.Spec.DeploymentTemplate.Metadata.GetName()) == 0 {
		err := errors.New("VerrazzanoGenericWorkload is missing required spec.deploymentTemplate.metadata.name")
	...
	// write out restart-version in generic deployment
	if err = r.restartGeneric(ctx, workload.Annotations[vzconst.RestartVersionAnnotation], &workload, log); err != nil {
		return reconcile.Result{}, err
	}
...
// convertWorkloadToDeployment converts a VerrazzanoGenericWorkload into a Deployment.
func (r *Reconciler) convertWorkloadToDeployment(workload *vzapi.VerrazzanoGenericWorkload, log vzlog.VerrazzanoLogger) (*appsv1.Deployment, error) {
	d := &appsv1.Deployment{
...
func (r *Reconciler) createServiceFromDeployment(workload *vzapi.VerrazzanoGenericWorkload, deploy *appsv1.Deployment, log vzlog.VerrazzanoLogger) (*corev1.Service, error) {
...
func passLabelAndAnnotation(workload *vzapi.VerrazzanoGenericWorkload, deploy *appsv1.Deployment) {
...
// addMetrics adds the labels and annotations needed for metrics to the Generic resource annotations which are propagated to the individual Generic pods.
func (r *Reconciler) addMetrics(ctx context.Context, log vzlog.VerrazzanoLogger, namespace string, workload *vzapi.VerrazzanoGenericWorkload, generic *appsv1.Deployment) error {
	...
	if generic.Spec.Template.Labels == nil {
		generic.Spec.Template.Labels = make(map[string]string)
	}

	if generic.Spec.Template.Annotations == nil {
		generic.Spec.Template.Annotations = make(map[string]string)
	}

	labels := metricstrait.MutateLabels(metricsTrait, nil, generic.Spec.Template.Labels)
	annotations := metricstrait.MutateAnnotations(metricsTrait, traitDefaults, generic.Spec.Template.Annotations)

	finalLabels := mergeMapOverrideWithDest(generic.Spec.Template.Labels, labels)
	log.Debugf("Setting labels on %s: %v", workload.Name, finalLabels)
	generic.Spec.Template.Labels = finalLabels
	finalAnnotations := mergeMapOverrideWithDest(generic.Spec.Template.Annotations, annotations)
	log.Debugf("Setting annotations on %s: %v", workload.Name, finalAnnotations)
	generic.Spec.Template.Annotations = finalAnnotations

	return nil
}

func (r *Reconciler) restartGeneric(ctx context.Context, restartVersion string, workload *vzapi.VerrazzanoGenericWorkload, log vzlog.VerrazzanoLogger) error {
