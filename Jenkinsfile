...
pipeline {
    ...
    agent {
       docker {
            ...
            label "CFGTST_tvlaent-7_2.0-large-phx"
        }
    }
    ...
    stages {
       ...
       stage('Verrazzano development version check') {
        ...
        stage('Scan Image') {
            ...
            steps {
                script {
                    ...
                    scanContainerImage "${env.DOCKER_REPO}/${env.DOCKER_NAMESPACE}/${DOCKER_AUTHPROXY_IMAGE_NAME}:${DOCKER_IMAGE_TAG}"
                }
            }
            post {
                failure {
        ...
        stage('Kind Acceptance Tests on 1.27') {
            when {
                allOf {
                    not { buildingTag() }
                    anyOf {
