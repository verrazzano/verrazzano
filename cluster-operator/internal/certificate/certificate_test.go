...
func TestSetupCertificates(t *testing.T) {
	...
	if a.NoError(err) {
		...
		if a.NoError(err) {
			...
			a.Equal(fmt.Sprintf("%s.%s.svc", OperatorName, OperatorNamespace), cert.DNSNames[0])
		}
	}
}
...
// TestUpdateValidatingWebhookConfiguration tests that the CA Bundle is updated in the verrazzano-cluster-operator
...
func TestUpdateValidatingWebhookConfiguration(t *testing.T) {
	...
	webhook := adminv1.ValidatingWebhookConfiguration{
		...
		ObjectMeta: metav1.ObjectMeta{
			Name: VerrazzanoManagedClusterValidatingWebhookName,
		},
	...
	err = UpdateValidatingWebhookConfiguration(kubeClient, &caCert, VerrazzanoManagedClusterValidatingWebhookName)
	a.Nil(err, "error should not be returned updating validation webhook configuration")

	updatedWebhook, _ := kubeClient.AdmissionregistrationV1().ValidatingWebhookConfigurations().Get(context.TODO(), VerrazzanoManagedClusterValidatingWebhookName, metav1.GetOptions{})
