...
const (
	...
	OperatorName = "verrazzano-cluster-operator"
...
const (
	// VerrazzanoManagedClusterValidatingWebhookName is the resource name for the VMC ValidatingWebhook
	VerrazzanoManagedClusterValidatingWebhookName = "verrazzano-cluster-operator-webhook"
)
...
func SetupCertificates(certDir string) (*bytes.Buffer, error) {
	...
	err = writeFile(fmt.Sprintf("%s/%s", certDir, CertKey), serverCertPEM)
	if err != nil {
		return nil, err
	}

	err = writeFile(fmt.Sprintf("%s/%s", certDir, PrivKey), serverPrivKeyPEM)
	if err != nil {
		return nil, err
	}
