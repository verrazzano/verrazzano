...
func (r *VerrazzanoManagedClusterReconciler) getJaegerOpenSearchConfig(vzList *vzapi.VerrazzanoList) (*jaegerOpenSearchConfig, error) {
	...
	// If the Jaeger OpenSearch URL is the default URL, use VZ OpenSearch ingress URL.
...
func (r *VerrazzanoManagedClusterReconciler) getJaegerSpecConfig(vzList *vzapi.VerrazzanoList) (jaegerSpecConfig, error) {
	...
	for _, vz := range vzList.Items {
		...
		if canUseVZOpenSearchStorage(vz, r.Client) {
...
func canUseVZOpenSearchStorage(vz vzapi.Verrazzano, client client.Client) bool {
	if ok, _ := vzcr.IsOpenSearchEnabled(&vz, client); ok && vzcr.IsKeycloakEnabled(&vz) && vzcr.IsAuthProxyEnabled(&vz) {
		return true
	}
	return false
}
