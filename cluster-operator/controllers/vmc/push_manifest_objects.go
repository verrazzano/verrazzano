...
func (r *VerrazzanoManagedClusterReconciler) pushManifestObjects(ctx context.Context, rancherEnabled bool, vmc *clusterapi.VerrazzanoManagedCluster) (bool, error) {
	pushed := false
	if rancherEnabled {
		secretsPushed, err := r.pushRegistrationAndAgentSecrets(vmc)
		if err != nil {
			return secretsPushed, err
		}
		pushed = pushed || secretsPushed
	}
	if vmc.Status.ClusterRef != nil {
		manifestPushed, err := r.pushManifestViaCAPI(ctx, vmc, rancherEnabled)
		if err != nil {
			return manifestPushed, err
		}
		pushed = pushed || manifestPushed
	}
	return pushed, nil
}

func (r *VerrazzanoManagedClusterReconciler) pushRegistrationAndAgentSecrets(vmc *clusterapi.VerrazzanoManagedCluster) (bool, error) {
...
func (r *VerrazzanoManagedClusterReconciler) pushManifestViaCAPI(ctx context.Context, vmc *clusterapi.VerrazzanoManagedCluster, rancherEnabled bool) (bool, error) {
