...
// TestDeleteSecretRancherProxy tests the delete of secrets through the Rancher proxy
// GIVEN a secret to be deleted on the managed cluster
//
//	WHEN the deleteSecretRancherProxy is called
//	THEN the secret is deleted from the managed cluster
func TestDeleteSecretRancherProxy(t *testing.T) {
	a := asserts.New(t)
	...
	tests := []struct {
		...
		{
			name:   "test secret doesn't exist",
			f:      func() error { return nil },
			mock:   addDeleteMock(mocks.NewMockRequestSender(mocker), &secret, clusterID, http.StatusNotFound, fmt.Errorf("not found")),
			result: false,
			retErr: false,
		},
		{
			...
			f:      func() error { return nil },
			mock:   addDeleteMock(mocks.NewMockRequestSender(mocker), &secret, clusterID, http.StatusOK, nil),
			result: true,
			retErr: false,
		},
		{
			name:   "test secret failed to delete",
			f:      func() error { return nil },
			mock:   addDeleteMock(mocks.NewMockRequestSender(mocker), &secret, clusterID, http.StatusConflict, fmt.Errorf("something wrong")),
			result: false,
			retErr: true,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			rancherutil.RancherHTTPClient = tt.mock
			result, err := deleteSecretRancherProxy(&secret, &rancherutil.RancherConfig{}, clusterID, vzlog.DefaultLogger())
			if tt.retErr {
				a.NotNil(err)
			} else {
				a.Nil(err)
