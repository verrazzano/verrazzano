...
function _logging_action_success() {
  ...
  if _logging_action_color_enabled; then
    format="${beglin}%-71s[${_LOGGING_ACTION_SUCCESS_CODES}%6s${_LOGGING_ACTION_NORMAL_CODES}]"
  fi
...
function _logging_action_failure() {
  ...
  if _logging_action_color_enabled; then
    format="${beglin}%-71s[${_LOGGING_ACTION_FAILURE_CODES}%6s${_LOGGING_ACTION_NORMAL_CODES}]"
  fi
